...
export default function Navbar() {
  ...
  const headerStyle: React.CSSProperties = scrolled
    ? {
        ...
        borderBottom: '1px solid rgba(201,151,58,0.22)',
    ...
    : {
        background: '#0D1F15',
        backdropFilter: 'none',
        borderBottom: '1px solid rgba(13,35,24,0.9)',
        boxShadow: 'none',
        transition: 'all 0.4s ease',
      }
  ...
  const iconBtnHover = (e: React.MouseEvent<HTMLElement>, enter: boolean) => {
    e.currentTarget.style.background = enter ? 'rgba(201,151,58,0.12)' : 'rgba(255,255,255,0.06)'
    e.currentTarget.style.borderColor = enter ? 'rgba(201,151,58,0.5)' : 'rgba(201,151,58,0.22)'
  }
        ...
        <div className="navbar-inner" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', height: 76, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          ...
          <Link href="/" style={{ display: 'flex', alignItems: 'center', textDecoration: 'none', flexShrink: 0 }} onClick={() => setOpen(false)}>
            <div className="navbar-logo" style={{
              ...
              boxShadow: scrolled ? '0 2px 12px rgba(0,0,0,0.3)' : '0 1px 4px rgba(0,0,0,0.15)',
            ...
            {links.map(l => {
              ...
              return (
                <Link key={l.href} href={l.href} style={{
                  ...
                  padding: '6px 10px',
                  borderRadius: 8,
                  background: active ? 'rgba(201,151,58,0.14)' : 'transparent',
                  border: active ? '1px solid rgba(201,151,58,0.28)' : '1px solid transparent',
                ...
                onMouseEnter={e => {
                  if (!active) {
                    ...
                    e.currentTarget.style.color = '#F2EEE6'
                  }
                }}
                onMouseLeave={e => {
                  if (!active) {
                    e.currentTarget.style.background = 'transparent'
                    e.currentTarget.style.color = '#8FC9A8'
                  }
                }}>
                  {l.label}
                </Link>
          ...
          <div className="desktop-cta" style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            <button
              onClick={toggle}
              aria-label={theme === 'dark' ? t('nav.modeJour') : t('nav.modeNuit')}
              style={iconBtnStyle}
              onMouseEnter={e => iconBtnHover(e, true)}
              onMouseLeave={e => iconBtnHover(e, false)}
            >
              {theme === 'dark' ? (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#E8B84B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {user ? (
              <div style={{ position: 'relative' }}>
                <button
                  ...
                  style={{
                    ...
                    background: menuUser ? 'rgba(201,151,58,0.15)' : 'rgba(255,255,255,0.06)',
                ...
                >
                  ...
                  <span style={{ color: '#F2EEE6', fontSize: 13, fontFamily: 'var(--font-dm-sans), Arial, sans-serif' }}>{user.prenom}</span>
                  <span style={{ color: '#8FC9A8', fontSize: 9, transition: 'transform 0.2s', transform: menuUser ? 'rotate(180deg)' : 'rotate(0)' }}>▼</span>
                </button>
              ...
              <>
                <Link href="/connexion" style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#8FC9A8', fontSize: 13, fontFamily: 'var(--font-dm-sans), Arial, sans-serif', textDecoration: 'none', padding: '6px 10px', borderRadius: 8, border: '1px solid transparent', transition: 'all 0.2s', whiteSpace: 'nowrap' }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = '#F2EEE6'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)' }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#8FC9A8'; e.currentTarget.style.borderColor = 'transparent' }}
                >
