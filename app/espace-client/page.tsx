...
const STATUT_COLORS: Record<string, { bg: string; color: string }> = {
  ...
  confirmee:    { bg: '#2D6A4F30', color: '#2D6A4F' },
  en_livraison: { bg: '#95D5B230', color: '#1A3C2E' },
  livree:       { bg: '#1A3C2E30', color: '#1A3C2E' },
  ...
  return (
    <div style={{ marginTop: 16, padding: '16px 0 8px' }}>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', marginBottom: 8 }}>
        {/* Ligne de fond */}
        <div style={{ position: 'absolute', top: '50%', left: '12px', right: '12px', height: 4, background: '#E5E7EB', borderRadius: 2, transform: 'translateY(-50%)', zIndex: 0 }} />
          ...
          return (
            <div key={etape.key} style={{ flex: 1, display: 'flex', justifyContent: i === 0 ? 'flex-start' : i === ETAPES.length - 1 ? 'flex-end' : 'center', zIndex: 2 }}>
              <div style={{
                ...
                borderRadius: '50%',
                background: fait ? (actuelle ? '#2D6A4F' : '#52B788') : '#fff',
                border: `3px solid ${fait ? '#2D6A4F' : '#D1D5DB'}`,
        ...
        {ETAPES.map((etape, i) => {
          ...
          return (
            <div key={etape.key} style={{ flex: 1, textAlign: i === 0 ? 'left' : i === ETAPES.length - 1 ? 'right' : 'center', paddingTop: 6 }}>
              <div style={{ fontSize: 11, fontWeight: actuelle ? 700 : 500, color: fait ? '#1A3C2E' : '#9CA3AF', fontFamily: 'Arial, sans-serif' }}>
                {etape.label}
              </div>
            </div>
          )
      ...
      {indexActuel >= 0 && (
        <div style={{ marginTop: 10, background: '#F0FAF4', borderRadius: 8, padding: '8px 12px', borderLeft: '3px solid #2D6A4F' }}>
          <p style={{ fontSize: 13, color: '#1A3C2E', fontFamily: 'Arial, sans-serif', margin: 0 }}>
...
export default function EspaceClient() {
  ...
  if (!user) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#F5F0E8', gap: 16 }}>
        ...
        <p style={{ fontSize: 14, color: '#7A6A5A', fontFamily: 'Arial, sans-serif' }}>Chargement de votre espace…</p>
  ...
  return (
    <>
      ...
      <section style={{ background: '#1A3C2E', padding: '32px 24px', borderBottom: '1px solid #0D2318' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ width: 56, height: 56, background: '#C9973A', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 700, color: '#1A3C2E', fontFamily: 'Arial, sans-serif', flexShrink: 0 }}>
              {initiales}
            </div>
            <div>
              <p style={{ fontSize: 13, color: '#95D5B2', fontFamily: 'Arial, sans-serif', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 3 }}>Espace client</p>
          ...
          <button
            ...
            style={{ background: 'transparent', border: '1px solid #2D6A4F', color: '#95D5B2', padding: '10px 20px', borderRadius: 6, fontSize: 14, fontFamily: 'Arial, sans-serif', cursor: 'pointer' }}
          >
            Se déconnecter
          </button>
        </div>
      ...
      <section style={{ background: '#F5F0E8', padding: '40px 24px', minHeight: '60vh' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div className="dashboard-grid" style={{ display: 'grid', gap: 24, alignItems: 'start' }}>
            ...
            {/* Infos personnelles */}
            <div style={{ background: '#fff', border: '0.5px solid #D4C9B0', borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
              ...
              <div style={{ padding: '20px 22px' }}>
                {[
                  ...
                  <div key={info.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #F0EBE0', flexWrap: 'wrap', gap: 4 }}>
                    <span style={{ fontSize: 14, color: '#7A6A5A', fontFamily: 'Arial, sans-serif' }}>{info.label}</span>
                    <span style={{ fontSize: 14, fontWeight: 700, color: '#1A3C2E', fontFamily: 'Arial, sans-serif' }}>{info.value}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ background: '#1A3C2E', border: '1px solid #2D6A4F', borderRadius: 14, padding: '24px 22px' }}>
                ...
                <p style={{ fontSize: 14, color: '#95D5B2', fontFamily: 'Arial, sans-serif', lineHeight: 1.7, marginBottom: 16 }}>
              ...
              {/* Laisser un avis */}
              <div style={{ background: '#fff', border: '0.5px solid #D4C9B0', borderRadius: 14, padding: '24px 22px' }}>
                <h2 style={{ fontSize: 16, fontWeight: 700, color: '#1A3C2E', fontFamily: 'Arial, sans-serif', marginBottom: 10 }}>⭐ Laisser un témoignage</h2>
                <p style={{ fontSize: 14, color: '#7A6A5A', fontFamily: 'Arial, sans-serif', lineHeight: 1.7, marginBottom: 16 }}>
              ...
              {/* Contact */}
              <div style={{ background: '#fff', border: '0.5px solid #D4C9B0', borderRadius: 14, padding: '24px 22px' }}>
                <h2 style={{ fontSize: 16, fontWeight: 700, color: '#1A3C2E', fontFamily: 'Arial, sans-serif', marginBottom: 10 }}>✉️ Besoin d'aide ?</h2>
                <p style={{ fontSize: 14, color: '#7A6A5A', fontFamily: 'Arial, sans-serif', lineHeight: 1.7, marginBottom: 16 }}>
                  Notre équipe est disponible Lun–Sam 8h–18h pour toute question.
                </p>
                <div style={{ display: 'flex', gap: 10 }}>
                  <Link href="/contact" style={{ flex: 1, background: '#F5F0E8', border: '1px solid #D4C9B0', color: '#1A3C2E', fontSize: 14, fontFamily: 'Arial, sans-serif', padding: '10px', borderRadius: 6, textDecoration: 'none', textAlign: 'center', fontWeight: 700 }}>
                    Formulaire
                  </Link>
                  <a href="tel:+2290195967762" style={{ flex: 1, background: '#F5F0E8', border: '1px solid #D4C9B0', color: '#1A3C2E', fontSize: 14, fontFamily: 'Arial, sans-serif', padding: '10px', borderRadius: 6, textDecoration: 'none', textAlign: 'center', fontWeight: 700 }}>
                    📞 Appeler
                  </a>
                </div>
              </div>
          ...
          <div style={{ marginTop: 32 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: '#1A3C2E', fontFamily: 'Arial, sans-serif' }}>
                🛒 Mes commandes
              </h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <button
                  ...
                  style={{ fontSize: 13, color: '#2D6A4F', background: '#F0FAF4', border: '1px solid #2D6A4F40', borderRadius: 8, padding: '6px 12px', cursor: 'pointer', fontFamily: 'Arial, sans-serif', fontWeight: 600 }}
                >
                  🔄 Actualiser
                </button>
              </div>
            ...
            {cmdLoading ? (
              <div style={{ background: '#fff', border: '0.5px solid #D4C9B0', borderRadius: 14, padding: 24, textAlign: 'center', color: '#7A6A5A', fontFamily: 'Arial, sans-serif', fontSize: 14 }}>
                Chargement de vos commandes…
              </div>
            ) : commandes.length === 0 ? (
              <div style={{ background: '#fff', border: '0.5px solid #D4C9B0', borderRadius: 14, padding: 32, textAlign: 'center' }}>
                <p style={{ color: '#7A6A5A', fontFamily: 'Arial, sans-serif', fontSize: 14, marginBottom: 16 }}>Vous n'avez pas encore passé de commande.</p>
                <Link href="/boutique" className="btn-gold" style={{ fontSize: 14 }}>Découvrir la boutique →</Link>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {commandes.map(cmd => {
                  const st         = STATUT_COLORS[cmd.statut] || { bg: '#eee', color: '#333' }
                  const isSelected = selectedCmd === cmd.id
                  return (
                    <div key={cmd.id} style={{ background: '#fff', border: '0.5px solid #D4C9B0', borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                          <span style={{ fontSize: 14, fontWeight: 700, color: '#1A3C2E', fontFamily: 'Arial, sans-serif' }}>Commande #{cmd.id}</span>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                          ...
                          <span style={{ fontSize: 12, color: '#7A6A5A', fontFamily: 'Arial, sans-serif' }}>{new Date(cmd.date_commande).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                          <span style={{ fontSize: 14, color: '#7A6A5A' }}>{isSelected ? '▲' : '▼'}</span>
                        </div>
                      </div>
                      ...
                      {isSelected && (
                        <div style={{ padding: '0 20px 20px', borderTop: '1px solid #F0EBE0' }}>
                          ...
                          <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
                            <p style={{ fontSize: 13, fontWeight: 700, color: '#1A3C2E', fontFamily: 'Arial, sans-serif', marginBottom: 6 }}>Détail :</p>
                            {cmd.lignes?.map(l => (
                              <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontFamily: 'Arial, sans-serif', color: '#1A3C2E', padding: '6px 10px', background: '#F9F6F0', borderRadius: 8 }}>
                            ...
                            <div style={{ borderTop: '1px solid #F0EBE0', paddingTop: 10, marginTop: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                              <span style={{ fontSize: 13, color: '#7A6A5A', fontFamily: 'Arial, sans-serif' }}>📍 {cmd.ville_livraison}</span>
                              <span style={{ fontSize: 13, color: '#7A6A5A', fontFamily: 'Arial, sans-serif' }}>💳 {cmd.paiement_affiche}</span>
                              <span style={{ fontSize: 14, fontWeight: 700, color: '#1A3C2E', fontFamily: 'Arial, sans-serif' }}>Total : {Number(cmd.total).toLocaleString()} FCFA</span>
                            </div>
                          </div>
