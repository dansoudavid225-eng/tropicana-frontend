...
const STATUT_COLORS: Record<string, { bg: string; color: string }> = {
  en_attente: { bg: '#FFF7ED', color: '#C2410C' }, confirmee: { bg: '#F0FDF4', color: '#15803D' },
  en_livraison: { bg: '#EFF6FF', color: '#1D4ED8' }, livree: { bg: '#F0FDF4', color: '#166534' }, annulee: { bg: '#FFF1F2', color: '#BE123C' }
}
...
const NAV: { s: Section; label: string; group: string }[] = [
  ...
  { s: 'alertes',    label: '⚠️ Alertes stock',  group: 'Gestion' },
...
const IS: React.CSSProperties = { width: '100%', border: '1.5px solid #E2E8F0', borderRadius: 8, padding: '9px 12px', fontSize: 14, background: '#F8FAFC', boxSizing: 'border-box', outline: 'none', fontFamily: 'inherit' }
const CS: React.CSSProperties = { background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: 20, boxShadow: '0 1px 3px rgba(0,0,0,0.04)', marginBottom: 16 }
const BG: React.CSSProperties = { background: '#2D6A4F', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }
const BR: React.CSSProperties = { background: '#FEF2F2', color: '#DC2626', border: '1px solid #FECACA', borderRadius: 8, padding: '7px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }
const BGR: React.CSSProperties = { background: '#F1F5F9', color: '#64748B', border: '1px solid #E2E8F0', borderRadius: 8, padding: '7px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }

function Loader() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 60, gap: 12 }}>
      <div style={{ width: 24, height: 24, borderRadius: '50%', border: '3px solid #E2E8F0', borderTopColor: '#2D6A4F', animation: 'spin .7s linear infinite' }} />
...
function Empty({ msg }: { msg: string }) {
  return <div style={{ textAlign: 'center', padding: '48px 24px', color: '#94A3B8', fontSize: 14 }}>{msg}</div>
}
...
function Toast({ msg, ok }: { msg: string; ok: boolean }) {
  return (
    <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9999, background: ok ? '#2D6A4F' : '#DC2626', color: '#fff', padding: '12px 20px', borderRadius: 10, fontSize: 14, fontWeight: 600, boxShadow: '0 4px 20px rgba(0,0,0,.2)' }}>
      {msg}
    </div>
  )
}
...
function SaveBar({ onSave, saving, label = '' }: { onSave: () => void; saving: boolean; label?: string }) {
  return (
    <div style={{ position: 'sticky', bottom: 0, background: 'rgba(255,255,255,.95)', borderTop: '1px solid #E2E8F0', padding: '14px 0', display: 'flex', justifyContent: 'flex-end', marginTop: 8, zIndex: 10 }}>
...
function LoginScreen({ onLogin }: { onLogin: (t: string) => void }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0D2318,#1A3C2E)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 20, padding: 40, width: '100%', maxWidth: 380, boxShadow: '0 30px 80px rgba(0,0,0,.4)' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ fontSize: 40, marginBottom: 8, color: '#2D6A4F', fontWeight: 900 }}>PIO</div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1A3C2E', margin: '0 0 4px' }}>Admin Tropicana</h1>
          <p style={{ fontSize: 13, color: '#64748B', margin: 0 }}>Panneau administration</p>
        </div>
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {err && <div style={{ background: '#FEF2F2', color: '#DC2626', padding: '10px 14px', borderRadius: 8, fontSize: 13 }}>{err}</div>}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div style={CS}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1A3C2E' }}>Textes principaux</h3>
        ...
        <div style={CS}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1A3C2E' }}>Boutons</h3>
...
function SectionArguments({ token }: { token: string }) {
  ...
  return (
    <div>
      {toast && <Toast {...toast} />}
      <div style={CS}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1E293B' }}>Les 4 arguments</h3>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {items.map((item, i) => (
            <div key={i} style={{ border: '1px solid #E2E8F0', borderRadius: 10, padding: 14, background: '#F8FAFC' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#64748B', marginBottom: 10 }}>ARGUMENT {i + 1}</div>
...
function SectionPlante({ token }: { token: string }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div style={CS}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1A3C2E' }}>Textes section plante</h3>
        ...
        <div style={CS}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, margin: 0, color: '#1A3C2E' }}>Points forts</h3>
            <button onClick={addP} style={BG}>+ Ajouter</button>
          </div>
          {points.map((p, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
              <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#2D6A4F', color: '#fff', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{i + 1}</div>
    ...
    <div>
      ...
      <div style={CS}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: 0, color: '#1A3C2E' }}>Bienfaits ({items.length}) — sauvegardés en base de données</h3>
          <button onClick={add} style={BG}>+ Ajouter</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {items.map((item, i) => (
            <div key={item.id || i} style={{ display: 'grid', gridTemplateColumns: '40px 70px 1fr 2fr 60px auto', gap: 10, alignItems: 'center', padding: 12, background: '#F8FAFC', borderRadius: 10, border: '1px solid #E2E8F0' }}>
              <div style={{ width: 36, height: 36, background: '#2D6A4F', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700 }}>{i + 1}</div>
...
function SectionTasse({ token }: { token: string }) {
  ...
  return (
    <div>
      {toast && <Toast {...toast} />}
      <div style={CS}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1A3C2E' }}>Section Tasse - banniere immersive</h3>
...
function SectionFondateur({ token }: { token: string }) {
  ...
  return (
    <div>
      {toast && <Toast {...toast} />}
      <div style={CS}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1A3C2E' }}>Bloc Fondateur - section verte accueil</h3>
...
function SectionStats({ token }: { token: string }) {
  ...
  return (
    <div>
      {toast && <Toast {...toast} />}
      <div style={CS}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1A3C2E' }}>Bandeau dore</h3>
        <FL label="Texte du bandeau"><Inp value={bandeau} onChange={setBandeau} /></FL>
      </div>
      <div style={CS}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1A3C2E' }}>Les 4 chiffres cles</h3>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {stats.map((s, i) => (
            <div key={i} style={{ border: '1px solid #E2E8F0', borderRadius: 10, padding: 14, background: '#F8FAFC' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#64748B', marginBottom: 10 }}>CHIFFRE {i + 1}</div>
      ...
      <div style={CS}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: 0, color: '#1A3C2E' }}>Avis rapides</h3>
          <button onClick={addT} style={BG}>+ Ajouter</button>
        </div>
        {temos.map((t, i) => (
          <div key={i} style={{ border: '1px solid #E2E8F0', borderRadius: 10, padding: 14, background: '#F8FAFC', marginBottom: 10 }}>
...
function SectionHistoire({ token }: { token: string }) {
  ...
  return (
    <div>
      ...
      <div style={CS}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: 0, color: '#1A3C2E' }}>Chapitres ({items.length}) — sauvegardés en base de données</h3>
          <button onClick={add} style={BG}>+ Ajouter chapitre</button>
        </div>
        {items.map((item, i) => (
          <div key={item.id || i} style={{ border: '1px solid #E2E8F0', borderRadius: 12, padding: 16, marginBottom: 14, background: '#F8FAFC' }}>
        ...
        {articles.map(a => (
          <div key={a.id} style={CS}>
            <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, color: '#C9973A', fontWeight: 700, marginBottom: 4 }}>{a.categorie} · {a.date_publication} · {a.temps_lecture} · {a.publie ? '✅ Publié' : '⚠️ Brouillon'}</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#1E293B', marginBottom: 4 }}>{a.titre}</div>
                <div style={{ fontSize: 13, color: '#64748B' }}>{a.extrait?.substring(0, 120)}...</div>
              </div>
    ...
    <div>
      ...
      <div style={CS}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: 0, color: '#1A3C2E' }}>Partenaires ({items.length}) — base de données</h3>
          <button onClick={add} style={BG}>+ Ajouter</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 14 }}>
          {items.map((item, i) => (
            <div key={item.id || i} style={{ border: '1px solid #E2E8F0', borderRadius: 12, padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
...
function SectionAnnonces({ token }: { token: string }) {
  ...
  return (
    <div>
      ...
      <div style={CS}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: 0, color: '#1A3C2E' }}>Barre d annonces - haut de page</h3>
          <button onClick={add} style={BG}>+ Ajouter</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#C9973A', color: '#fff', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{i + 1}</div>
...
function SectionFooter({ token }: { token: string }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div style={CS}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1A3C2E' }}>Bande CTA doree</h3>
        ...
        <div style={CS}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px', color: '#1A3C2E' }}>Infos footer</h3>
...
function SectionCouleurs({ token }: { token: string }) {
  const { config, loading, saving, save, toast } = useConfig(token)
  const [cols, setCols] = useState({ couleur_vert: '#2D6A4F', couleur_or: '#C9973A', couleur_creme: '#F5F0E8', couleur_fonce: '#1A3C2E' })
  useEffect(() => {
    if (!loading) setCols({ couleur_vert: config.couleur_vert || '#2D6A4F', couleur_or: config.couleur_or || '#C9973A', couleur_creme: config.couleur_creme || '#F5F0E8', couleur_fonce: config.couleur_fonce || '#1A3C2E' })
  }, [loading, config])
  const colors = [
    ...
    <div>
      ...
      <div style={CS}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 16 }}>
          {colors.map(c => (
            <div key={c.k} style={{ border: '1px solid #E2E8F0', borderRadius: 12, overflow: 'hidden' }}>
              <div style={{ height: 80, background: cols[c.k], position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                ...
                <span style={{ color: '#fff', fontSize: 12, fontWeight: 700, textShadow: '0 1px 4px rgba(0,0,0,.5)', pointerEvents: 'none' }}>Cliquer pour changer</span>
              </div>
              <div style={{ padding: 14 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 24, height: 24, borderRadius: 6, background: cols[c.k], border: '2px solid #E2E8F0', flexShrink: 0 }} />
    ...
    <div>
      ...
      {sel && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 100, display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-end' }}>
          <div style={{ background: '#fff', width: '100%', maxWidth: 440, height: '100vh', overflowY: 'auto', padding: 28 }}>
            ...
            <div style={{ background: '#F8FAFC', borderRadius: 8, padding: 14, marginBottom: 12, fontSize: 13 }}>
            ...
            <div style={{ background: '#F8FAFC', borderRadius: 8, padding: 14, marginBottom: 12 }}>
              ...
              <div style={{ borderTop: '1px solid #E2E8F0', marginTop: 8, paddingTop: 8, display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: 15 }}>
                <span>Total</span>
                <span style={{ color: '#C9973A' }}>{fm(sel.total)}</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
              {Object.entries(STATUT_LABELS).map(([k, v]) => (
                <button key={k} onClick={() => chg(sel.id, k)} style={{ padding: '7px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', background: sel.statut === k ? '#1A3C2E' : '#F1F5F9', color: sel.statut === k ? '#fff' : '#475569', border: 'none' }}>{v}</button>
              ))}
            </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <input
              ...
              style={{ border: '1.5px solid #E2E8F0', borderRadius: 8, padding: '6px 12px', fontSize: 13 }}
            />
            <select
              value={filtreStatut}
            ...
            <button
              ...
              style={{ background: '#1D6F42', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
            >📊 Excel</button>
            <button
              onClick={() => window.open(`${API}/admin/commandes/?format=csv`, '_blank')}
              style={{ background: '#457B9D', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
            >📋 CSV</button>
          </div>
        </div>
        {cmds.length === 0 ? <Empty msg="Aucune commande" /> : (
              ...
              <thead>
                <tr style={{ background: '#F8FAFC' }}>
                  {['#', 'Client', 'Ville', 'Total', 'Statut', 'Date', ''].map(h => (
                    <th key={h} style={{ padding: '10px 12px', fontSize: 11, fontWeight: 700, color: '#64748B', textAlign: 'left', textTransform: 'uppercase' as const }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {cmds.map(c => (
                  <tr key={c.id} style={{ borderTop: '1px solid #F1F5F9' }}>
                    <td style={{ padding: '11px 12px', fontWeight: 700, color: '#64748B', fontSize: 13 }}>#{c.id}</td>
                    <td style={{ padding: '11px 12px', fontSize: 13, fontWeight: 600 }}>{c.nom_client}</td>
                    <td style={{ padding: '11px 12px', fontSize: 13, color: '#64748B' }}>{c.ville_livraison}</td>
                    <td style={{ padding: '11px 12px', fontSize: 13, fontWeight: 700, color: '#C9973A' }}>{fm(c.total)}</td>
                    <td style={{ padding: '11px 12px' }}><Badge statut={c.statut} /></td>
                    <td style={{ padding: '11px 12px', fontSize: 12, color: '#94A3B8' }}>{fd(c.date_commande)}</td>
                    <td style={{ padding: '11px 12px' }}>
                      <button onClick={() => setSel(c)} style={{ fontSize: 12, color: '#2D6A4F', background: 'none', border: '1px solid #D1FAE5', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontWeight: 600 }}>Details</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 16 }}>
              ...
              <span style={{ padding: '6px 14px', fontSize: 13, color: '#666' }}>Page {page}/{totalPages}</span>
        ...
        {prods.map(p => (
          <div key={p.id} style={CS}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              ...
              <span style={{ background: p.disponible ? '#D1FAE5' : '#FEE2E2', color: p.disponible ? '#065F46' : '#991B1B', fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 20 }}>{p.disponible ? 'En vente' : 'Hors stock'}</span>
            </div>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#C9973A', marginBottom: 6 }}>{fm(p.prix)} / {p.unite}</div>
            <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.5, margin: '0 0 14px' }}>{p.description?.substring(0, 80)}...</p>
            <div style={{ display: 'flex', gap: 8 }}>
    ...
    <div>
      ...
      {temos.length === 0 ? <Empty msg="Aucun temoignage" /> : temos.map(t => (
        <div key={t.id} style={{ ...CS, display: 'flex', gap: 16 }}>
          <div style={{ width: 42, height: 42, borderRadius: '50%', background: t.approuve ? '#D1FAE5' : '#FEE2E2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0, color: t.approuve ? '#065F46' : '#991B1B' }}>{t.approuve ? 'OK' : '?'}</div>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <div><span style={{ fontSize: 14, fontWeight: 700 }}>{t.nom}</span><span style={{ fontSize: 12, color: '#94A3B8', marginLeft: 8 }}>{t.ville}</span></div>
              <div style={{ color: '#F59E0B' }}>{'*'.repeat(t.note)}</div>
            </div>
            <p style={{ fontSize: 13, color: '#475569', margin: '0 0 10px', lineHeight: 1.6 }}>{t.texte}</p>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => toggle(t)} style={{ ...BG, fontSize: 12, padding: '6px 14px', background: t.approuve ? '#64748B' : '#2D6A4F' }}>{t.approuve ? 'Masquer' : 'Publier'}</button>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: sel ? '1fr 1fr' : '1fr', gap: 16 }}>
      <div style={CS}>
        ...
        {msgs.length === 0 ? <Empty msg="Aucun message" /> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {msgs.map(m => (
              <button key={m.id} onClick={() => { setSel(m); marquerLu(m.id) }} style={{ background: m.lu ? '#F8FAFC' : '#EFF6FF', border: `1px solid ${m.lu ? '#E2E8F0' : '#BFDBFE'}`, borderRadius: 10, padding: 14, cursor: 'pointer', textAlign: 'left' as const }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: 13, fontWeight: m.lu ? 500 : 700 }}>{m.nom}</span>
                  {!m.lu && <span style={{ background: '#2563EB', color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 10 }}>NOUVEAU</span>}
                </div>
                <div style={{ fontSize: 12, color: '#64748B' }}>{m.email} - {m.objet}</div>
              </button>
            ))}
          </div>
        )}
      ...
      {sel && (
        <div style={CS}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
            ...
            <button onClick={() => setSel(null)} style={{ background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: '#94A3B8' }}>X</button>
          </div>
          <div style={{ background: '#F8FAFC', borderRadius: 8, padding: 12, fontSize: 13, marginBottom: 10 }}>
            <div>Email : <a href={`mailto:${sel.email}`} style={{ color: '#2D6A4F' }}>{sel.email}</a></div>
            {sel.telephone && <div>Tel : {sel.telephone}</div>}
            <div>Objet : {sel.objet || '-'}</div>
          </div>
          <div style={{ background: '#F8FAFC', borderRadius: 8, padding: 14, fontSize: 14, lineHeight: 1.7, color: '#334155', marginBottom: 10 }}>{sel.message}</div>
          ...
          <thead>
            <tr style={{ background: '#F8FAFC' }}>
              {['Nom', 'Email', 'Telephone', 'Ville', 'Inscription', 'Role'].map(h => (
                <th key={h} style={{ padding: '10px 14px', fontSize: 11, fontWeight: 700, color: '#64748B', textAlign: 'left', textTransform: 'uppercase' as const }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.map(u => (
              <tr key={u.id} style={{ borderTop: '1px solid #F1F5F9' }}>
                ...
                <td style={{ padding: '12px 14px', fontSize: 13, color: '#64748B' }}>{u.telephone || '-'}</td>
                <td style={{ padding: '12px 14px', fontSize: 13, color: '#64748B' }}>{u.ville || '-'}</td>
                <td style={{ padding: '12px 14px', fontSize: 12, color: '#94A3B8' }}>{fd(u.date_inscription)}</td>
                <td style={{ padding: '12px 14px' }}>
                  <span style={{ background: u.is_staff ? '#FEF3C7' : '#F1F5F9', color: u.is_staff ? '#92400E' : '#64748B', fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20 }}>{u.is_staff ? 'Admin' : 'Client'}</span>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div style={CS}>
      ...
      {data.length === 0 ? <Empty msg="Aucun abonne" /> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {data.map(a => (
            <div key={a.email} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', background: '#F8FAFC', borderRadius: 8, fontSize: 13 }}>
              <span>{a.email}</span>
              <span style={{ color: '#94A3B8' }}>{fd(a.date_inscription)}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(155px,1fr))', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Commandes', val: stats.commandes, sub: `${stats.attente} en attente`, s: 'commandes' as Section },
          { label: 'CA livre', val: fm(stats.ca), sub: 'commandes livrees', s: 'commandes' as Section },
          { label: 'Messages', val: stats.messages, sub: `${stats.nonLus} non lus`, s: 'messages' as Section },
          { label: 'A moderer', val: stats.temos, sub: 'temoignages', s: 'temoignages' as Section },
          { label: 'Produits', val: stats.produits, sub: 'en catalogue', s: 'produits' as Section },
          { label: 'Membres', val: stats.users, sub: 'inscrits', s: 'utilisateurs' as Section },
        ].map(c => (
          <button key={c.label} onClick={() => setSection(c.s)} style={{ ...CS, cursor: 'pointer', textAlign: 'left' as const, marginBottom: 0 }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: '#1E293B', marginBottom: 2 }}>{c.val}</div>
...
function SectionPromo({ token }: { token: string }) {
  ...
  const CS: React.CSSProperties = { background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,.08)' }
  const IS: React.CSSProperties = { border: '1.5px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13, width: '100%', boxSizing: 'border-box' as const }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={CS}>
        ...
        <button onClick={creer} style={{ background: '#2D6A4F', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontWeight: 700, cursor: 'pointer' }}>+ Créer le code</button>
        {msg && <p style={{ marginTop: 8, fontSize: 13, color: '#2D6A4F' }}>{msg}</p>}
      </div>
      <div style={CS}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px' }}>Codes existants ({promos.length})</h3>
        {promos.map(p => (
          <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #F0F4F1' }}>
            <div>
              <span style={{ fontWeight: 700, fontFamily: 'monospace', fontSize: 14 }}>{p.code}</span>
              <span style={{ marginLeft: 10, fontSize: 12, color: '#666' }}>{p.valeur}{p.type_reduction === 'pourcentage' ? '%' : ' FCFA'} — {p.nb_utilisations}/{p.limite_utilisations || '∞'} utilisations</span>
              {!p.valide && <span style={{ marginLeft: 8, background: '#FEE2E2', color: '#DC2626', borderRadius: 4, padding: '2px 6px', fontSize: 11 }}>Expiré</span>}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => toggle(p.id, p.actif)} style={{ background: p.actif ? '#D1FAE5' : '#FEE2E2', color: p.actif ? '#065F46' : '#DC2626', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>{p.actif ? 'Actif' : 'Inactif'}</button>
              <button onClick={() => supprimer(p.id)} style={{ background: '#FEE2E2', color: '#DC2626', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 12, cursor: 'pointer' }}>Suppr.</button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={CS}>
        ...
        <button onClick={creer} style={{ background: '#2D6A4F', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontWeight: 700, cursor: 'pointer' }}>+ Ajouter</button>
        {msg && <p style={{ marginTop: 8, fontSize: 13, color: '#2D6A4F' }}>{msg}</p>}
      </div>
      <div style={CS}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px' }}>Villes configurées ({zones.length})</h3>
        {zones.map(z => (
          <div key={z.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #F0F4F1' }}>
            <div>
              <span style={{ fontWeight: 700 }}>{z.ville}</span>
              <span style={{ marginLeft: 10, fontSize: 12, color: '#666' }}>{z.prix.toLocaleString()} FCFA — {z.delai}</span>
            </div>
            <button onClick={() => toggle(z.id, z.disponible)} style={{ background: z.disponible ? '#D1FAE5' : '#FEE2E2', color: z.disponible ? '#065F46' : '#DC2626', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>{z.disponible ? 'Active' : 'Inactive'}</button>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={CS}>
        ...
        <button onClick={ajouter} style={{ background: '#DC2626', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontWeight: 700, cursor: 'pointer' }}>🚫 Bloquer</button>
        {msg && <p style={{ marginTop: 8, fontSize: 13, color: '#2D6A4F' }}>{msg}</p>}
      </div>
      <div style={CS}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px' }}>Éléments bloqués ({liste.length})</h3>
        {liste.length === 0 && <p style={{ color: '#999', fontSize: 13 }}>Aucun élément bloqué.</p>}
        {liste.map(b => (
          <div key={b.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #F0F4F1' }}>
            <div>
              <span style={{ background: '#FEE2E2', color: '#DC2626', borderRadius: 4, padding: '2px 6px', fontSize: 11, fontWeight: 700, marginRight: 8 }}>{b.type}</span>
              <span style={{ fontFamily: 'monospace', fontSize: 13 }}>{b.valeur}</span>
              {b.raison && <span style={{ marginLeft: 8, color: '#666', fontSize: 12 }}>— {b.raison}</span>}
            </div>
            <button onClick={() => retirer(b.id)} style={{ background: '#F1F5F9', color: '#64748B', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 12, cursor: 'pointer' }}>Retirer</button>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={CS}>
        ...
        <button onClick={sauvegarder} style={{ background: '#F59E0B', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontWeight: 700, cursor: 'pointer' }}>⚠️ Configurer l'alerte</button>
        {msg && <p style={{ marginTop: 8, fontSize: 13, color: '#2D6A4F' }}>{msg}</p>}
      </div>
      <div style={CS}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 16px' }}>Alertes configurées</h3>
        {alertes.length === 0 && <p style={{ color: '#999', fontSize: 13 }}>Aucune alerte configurée.</p>}
        {alertes.map((a: any) => (
          <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #F0F4F1' }}>
            <div>
              <span style={{ fontWeight: 700 }}>{a.produit_nom}</span>
              <span style={{ marginLeft: 8, fontSize: 12, color: '#666' }}>Seuil: {a.seuil} — Stock actuel: {a.stock_actuel}</span>
              {a.en_alerte && <span style={{ marginLeft: 8, background: '#FEF3C7', color: '#D97706', borderRadius: 4, padding: '2px 6px', fontSize: 11, fontWeight: 700 }}>⚠️ EN ALERTE</span>}
            </div>
          </div>
        ))}
      </div>
...
function SectionRapport({ token }: { token: string }) {
  ...
  const CS: React.CSSProperties = { background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,.08)' }
  const IS: React.CSSProperties = { border: '1.5px solid #E2E8F0', borderRadius: 8, padding: '8px 12px', fontSize: 13 }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={CS}>
        ...
        <p style={{ fontSize: 13, color: '#666', marginBottom: 16 }}>Générez un rapport complet des ventes (commandes, CA, détail) pour le mois de votre choix.</p>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <button onClick={telecharger} style={{ background: '#2D6A4F', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontWeight: 700, cursor: 'pointer' }}>
      ...
      <div style={CS}>
        ...
        <p style={{ fontSize: 13, color: '#666', marginBottom: 16 }}>Exportez toutes les commandes en Excel ou CSV pour analyse.</p>
        <div style={{ display: 'flex', gap: 12 }}>
          <button onClick={exportExcel} style={{ background: '#1D6F42', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontWeight: 700, cursor: 'pointer' }}>📊 Export Excel (.xlsx)</button>
          <button onClick={exportCsv}   style={{ background: '#457B9D', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontWeight: 700, cursor: 'pointer' }}>📋 Export CSV</button>
        </div>
      </div>
    </div>
  )
...
export default function AdminPanel() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F1F5F9', fontFamily: 'system-ui,-apple-system,sans-serif' }}>
      <aside style={{ width: open ? 220 : 60, flexShrink: 0, background: '#1A3C2E', display: 'flex', flexDirection: 'column', transition: 'width .22s ease', overflow: 'hidden', position: 'sticky', top: 0, height: '100vh' }}>
        <div style={{ padding: '16px 12px 12px', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 32, height: 32, background: '#C9973A', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 900, fontSize: 13, flexShrink: 0 }}>PP</div>
            {open && <div><div style={{ fontSize: 13, fontWeight: 700, color: '#F0EBE0' }}>Pio Pio</div><div style={{ fontSize: 10, color: '#6B9E7A' }}>Administration</div></div>}
          </div>
        </div>
          ...
          {['General', 'Site', 'Gestion'].map(g => (
            <div key={g}>
              ...
              {NAV.filter(n => n.group === g).map(({ s, label }) => (
                <button key={s} onClick={() => setSection(s)} title={!open ? label : undefined} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '9px 14px', background: section === s ? 'rgba(201,151,58,.15)' : 'transparent', borderLeft: `3px solid ${section === s ? '#C9973A' : 'transparent'}`, border: 'none', borderTop: 'none', borderRight: 'none', borderBottom: 'none', cursor: 'pointer', color: section === s ? '#F0EBE0' : '#95D5B2', fontSize: 12, fontWeight: section === s ? 700 : 400, whiteSpace: 'nowrap' as const, overflow: 'hidden' }}
                  ...
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: section === s ? '#C9973A' : 'rgba(255,255,255,.2)', flexShrink: 0 }} />
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,.08)', padding: '10px 10px', display: 'flex', gap: 6, flexDirection: open ? 'row' : 'column' as const, alignItems: 'center' }}>
          <a href="/" target="_blank" style={{ flex: open ? 1 : undefined, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', borderRadius: 6, padding: '7px 10px', fontSize: 12, color: '#95D5B2', textDecoration: 'none', fontWeight: 600 }}>{open ? 'Voir le site' : 'V'}</a>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <header style={{ background: '#fff', borderBottom: '1px solid #E2E8F0', padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 40, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <button onClick={() => setOpen(!open)} style={{ background: '#F1F5F9', border: '1px solid #E2E8F0', borderRadius: 8, width: 32, height: 32, cursor: 'pointer', fontSize: 13, color: '#64748B', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{open ? '<' : '>'}</button>
            <h1 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#1E293B' }}>{TITLES[section]}</h1>
          </div>
          <div style={{ fontSize: 12, color: '#94A3B8' }}>{new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</div>
        </header>
